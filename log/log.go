...
func (l *ZLogger) SetLogFile(fileName string) {
	l.prefix = filepath.Dir(fileName)
}
...
func (l *ZLogger) msg2File(ppFile **os.File, fileName *string, txt, tag string, when time.Time) (err error) {
	...
	if l.isFileDaily {
		newFileName = filepath.Join(l.prefix, fmt.Sprintf("%v%v.log", tag, when.Format("20060102")))
	} else {
		newFileName = filepath.Join(l.prefix, when.Format("20060102.log"))
	}
...
func (l *ZLogger) msgOut(logLevel int, txt string) {
	...
	if l.isAsynToFile {
		...
		_ = l.msgToFile(LogNode{when: now, msg: txt, level: logLevel})
	}
}
